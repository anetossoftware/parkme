...
class UpdateConstraintSet : ConstraintSet() {

    companion object {
        inline fun buildConstraintSet(block:UpdateConstraintSet.()->Unit) =
            UpdateConstraintSet().apply(block)
    }
    //add margin param in ConstraintSet.connect
    var margin: Int? = null
        get() {
            val result = field
    ...
    inline infix fun Int.startToStartOf(start: Int) =
        margin?.let {
            connect(this, START, start, START, it)
        } ?: connect(this, START, start, START)
    ...
    inline infix fun Int.endToEndOf(end: Int) =
        margin?.let {
